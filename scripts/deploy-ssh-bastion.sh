...
SSH_SERVICE_PORT=${SSH_SERVICE_PORT:-443}
SSH_TUNNEL_PORT=${SSH_TUNNEL_PORT:-8080}
...
Проверка на хосте:
  ...
  curl -i http://${SERVICE_INTERNAL_NODE_IP:-<node-ip>}:${SSH_SERVICE_NODE_PORT}  # ожидаем HTTP 404 от chisel-сервера
